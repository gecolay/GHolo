...
public class GInteraction {
    ...
    public @Nullable GInteractionAction getAction(int position) { return (position >= 0 && position < actions.size()) ? actions.get(position) : null; }
    ...
    public @NotNull GInteraction removeAction(int position) {
        if(position >= 0 && position < actions.size()) actions.remove(position);
        return this;
    }
