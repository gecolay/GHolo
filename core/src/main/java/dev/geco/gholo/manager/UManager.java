...
public class UManager {
    ...
    public void checkForUpdates() {
        ...
        for(Player player : Bukkit.getOnlinePlayers()) if(GPM.getPManager().hasPermission(player, "Update")) GPM.getMManager().sendMessage(player, "Plugin.plugin-update", "%Name%", GHoloMain.NAME, "%NewVersion%", spigotVersion, "%Version%", GPM.getDescription().getVersion(), "%Path%", GPM.getDescription().getWebsite());
        GPM.getMManager().sendMessage(Bukkit.getConsoleSender(), "Plugin.plugin-update", "%Name%", GHoloMain.NAME, "%NewVersion%", spigotVersion, "%Version%", GPM.getDescription().getVersion(), "%Path%", GPM.getDescription().getWebsite());
    }
    ...
    public void loginCheckForUpdates(Player Player) {
        ...
        checkVersion();
        if(latestVersion) return;
        GPM.getMManager().sendMessage(Player, "Plugin.plugin-update", "%Name%", GHoloMain.NAME, "%NewVersion%", spigotVersion, "%Version%", GPM.getDescription().getVersion(), "%Path%", GPM.getDescription().getWebsite());
    }
